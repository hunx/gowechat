...
//Redirect 跳转到网页授权
func (oauth *Oauth) Redirect(responser *http.Request, writer http.ResponseWriter, redirectURI, scope, state string) error {
	location, err := oauth.GetRedirectURL(redirectURI, scope, state)
	if err != nil {
		return err
	}
	//location 为完整地址，所以不需要request
	http.Redirect(writer, responser, location, 302)
	return nil
}
